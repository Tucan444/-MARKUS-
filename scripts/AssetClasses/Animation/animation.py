...
class Animation:
    def __init__(self, game: 'Game', frames: list[pygame.Surface], name: str, frame_durations: list[float], loop: bool=True, pong: bool=False,
                 reversed_: bool=False, flip_x: bool=False, flip_y: bool=False,
                 flipped_generated: bool=False, timeline: Timeline=None):
    ...
    @reversed.setter
    def reversed(self, reversed_: bool) -> None:
        if reversed_ is not self._reversed:
            self.reverse()
    ...
    def update_time(self) -> None:
        ...
        self.animation_time += self.timeline.dt * self.frame_direction
        ...
        self.frame_time += self.timeline.dt
    ...
    def _advance_current_frame(self) -> None:
        ...
        beyond_end: bool = self.current_frame >= len(self.frames) or self.current_frame < 0
        if beyond_end and not self.loop:
            self.ended = True
            self.current_frame = len(self.frames) - 1 if self.current_frame >= len(self.frames) else 0
    ...
    def clone(self) -> 'Animation':
        animation: Animation = Animation(self.game, self.frames, self.name, self.frame_durations, self.loop, self.pong,
                                   self.reversed, self.flip_x, self.flip_y, self.flipped_generated)
